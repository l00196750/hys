...
public class FileUploadApi implements BaseApi {
    ...
    public List<FileUploadResponse> upload(HttpServletRequest request) throws IOException {
        ...
        String saveParentDir = new StringBuilder().append(request.getSession().getServletContext().getRealPath("/"))
                .append("upload").append(File.separator).append(LocalDates.now(null)).append(File.separator).toString();
    ...
    private FileUploadResponse saveUploadFile(String saveParentDir, MultipartFile multipartFile)
        ...
        String saveFileName = new StringBuilder().append(LocalDates.now(null)).append(".").append(IdWorker.nextId())
                .append(".").append(StringUtils.substringAfterLast(rawFilename, ".")).toString();
